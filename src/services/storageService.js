...
export const getWardPresentations = () => {
  try {
    ...
    if (!saved) {
      // Initialize with mockConditions if completely empty
...
export const getCachedGuide = (diseaseName) => {
  try {
    ...
    // Be lenient with matching by normalizing strings
    return guides.find(g => g.conditionName.toLowerCase() === diseaseName.toLowerCase()) || null;
  } catch (e) {
    console.error("Storage error", e);
    return null;
  }
...
export const saveConditionGuide = (guide) => {
  ...
  try {
    ...
    // Check if it exists in presentations
    ...
    if (!exists) {
      presentationsArray.push({
        ...
        // Automatically make it a red flag presentation if it has 3 or more extreme red flags
